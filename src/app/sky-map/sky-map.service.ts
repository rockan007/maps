...
export class SkyMapService {
  ...
  getSearchResult(keywords: string): Observable<any> {
    ...
    const result = this.http.get(this.searchUrl, { params });
    return result;
  }
  // 点击地点的变化监听
  newCurrentMapPointerSubjectInstance(): Subject<MapPointer> {
  ...
  // 地图变化监听
  newMapSubjectInstance(): Subject<any> {
  ...
  transformGCJ2WGS(gcjLat, gcjLon): { lat: number, lng: number } {
  ...
  delta(lat: number, lon: number): { lat: number, lon: number } {
